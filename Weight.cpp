...
/////////////////////////////////// Static Methods
//////////////////////////////////////
float Weight::fromKilogramsToPounds(float weightInKilos)

    noexcept {
  return weightInKilos / KILOS_IN_A_POUND;
}

float Weight::fromPoundsToKilograms(float weightInPounds)

    noexcept {
  return weightInPounds * KILOS_IN_A_POUND;
}

float Weight::fromSlugsToPounds(float weightInSlugs)

    noexcept {
  return weightInSlugs / SLUGS_IN_A_POUND;
}

float Weight::fromPoundsToSlugs(float weightInPounds)

    noexcept {
  return weightInPounds * SLUGS_IN_A_POUND;
}

float Weight::convertWeight(float fromWeight, Weight::UnitOfWeight fromUnit,
                            Weight::UnitOfWeight toUnit) noexcept {
  switch (fromUnit) {
  case POUNDS:
    switch (toUnit) {
    case POUNDS:
...
std::ostream &operator<<(std::ostream &lhs_stream,
                         const Weight::UnitOfWeight rhsUnit) {
  switch (rhsUnit) {
  case Weight::POUNDS:
...
//////////////////////////////////////
bool Weight::weightIsKnown() const
...
void Weight::setWeight(float newWeight, Weight::UnitOfWeight weightUnits) {
  if (weightIsValid(newWeight)) {
    float convertedWeight =
        convertWeight(newWeight, getUnitOfWeight(), weightUnits);
    _weight = convertedWeight;
  }
}

bool Weight::weightIsValid(float checkWeight) const

    noexcept {
  return (checkWeight > 0 && checkWeight != UNKNOWN_WEIGHT &&
          checkWeight <= _maxWeight)
             ? true
             : false;
}

bool Weight::maxWeightIsValid(float checkMaxWeight) const

    noexcept {
  return (checkMaxWeight > 0 && checkMaxWeight < DEFAULT_MAX_WEIGHT) ? true
                                                                     : false;
}
...
bool Weight::validate() const
    ...
    noexcept {
  return (weightIsValid(_weight) && maxWeightIsValid(_maxWeight)) ? true
                                                                  : false;
}
...
void Weight::dump() const
  ...
  std::cout << "Max Weight: " << _maxWeight << " " << _unitOfWeight
            << std::endl;
}
